...
def get_text_pc(i, j, gb, l1, l2): 
    ...
    if winner(board, "X"): 
        gb.destroy() 
        x = False
        box = messagebox.showinfo("WELL PLAYED", "You won the match") 
    elif winner(board, "O"): 
        gb.destroy() 
        x = False
        box = messagebox.showinfo("GAME OVER!!", "Try Again") 
    elif(isfull()): 
        gb.destroy() 
        x = False
        box = messagebox.showinfo("GAME TIED", "Try Again") 
    if(x): 
        if sign % 2 != 0: 
            move = pc() 
